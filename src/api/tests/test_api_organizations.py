...
def test_read_organization_usage_pagination():
    ...
    page = 1
    size = 15
    response = client.get(f"api/v1/organization/{good_org_id}/usage?start={start}&end={end}&every={every}&page={page}&size={size}")
...
def test_read_inexistent_organization_usage():
    ...
    page = 1
    size = 15
    response = client.get(f"api/v1/organization/{bad_org_id}/usage?start={start}&end={end}&every={every}&page={page}&size={size}")
...
def test_read_organization_usage_inexistent_page():
    ...
    size = 15
    response = client.get(f"api/v1/organization/{good_org_id}/usage?start={start}&end={end}&every={every}&size={size}")
    assert response.status_code == 200
    resp = response.json()
    assert "items" in resp
    assert "total" in resp
...
def test_read_organization_usage_inexistent_page_and_size():
    ...
    response = client.get(f"api/v1/organization/{good_org_id}/usage?start={start}&end={end}&every={every}")
    assert response.status_code == 200
    resp = response.json()
    assert "items" in resp
    assert "total" in resp
...
def test_read_organization_usage_inexistent_every():
    ...
    response = client.get(f"api/v1/organization/{good_org_id}/usage?start={start}&end={end}")
    assert response.status_code == 422

def test_read_organization_usage_bad_start():
    """Test if get 422 on bad parameter 'start'"""

    response = client.get(f"api/v1/organization/{good_org_id}/usage?start={bad_start}&end={end}")
    assert response.status_code == 422

def test_read_organization_usage_bad_end():
    """Test if get 422 on bad parameter 'end'"""

    response = client.get(f"api/v1/organization/{good_org_id}/usage?start={start}&end={bad_end}")
    assert response.status_code == 422
